...
<%@ page import="com.spring.drone.news.JobNewsPaginationVO"%> 
...
<% 
   ...
   ArrayList<NewsVO> jobNewsList = (ArrayList<NewsVO>)request.getAttribute("jobNewsList");
	JobNewsPaginationVO jobNewsPaginationVO = (JobNewsPaginationVO)request.getAttribute("jobNewsPaginationVO");
   SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
   int number = jobNewsPaginationVO.getNumber();
...
<div class="container">
  ...
  <div class="row mt-1 mb-1">
		<div class="col-lg-12 line_color_yellow_b text_align_center mb-5">
			...
			<p class="korean_center text_border">드론구직 뉴스 아카이브</p>
...
<%
      if(jobNewsPaginationVO.getCount() == 0){
%>    
...
<%
         for(int i=0;i < jobNewsList.size();i++){
            NewsVO vo = jobNewsList.get(i);   
%>      
            <tr class="d-flex">
               <td class="col-sm-1">
                  <%=number%>
               ...
               <td class="col-sm-8 pl-0 td_no_overflow">
		          <a href="template.templ?page=getNews.news?num=<%=vo.getNum() %>&pageNum=<%=jobNewsPaginationVO.getCurrentPage() %>&number=<%=jobNewsPaginationVO.getNumber() %>&readcount=<%=vo.getReadcount()%>">
                     <%=vo.getTitle() %>
                  </a>
                 </td>
...
<%
      ...
      if(jobNewsPaginationVO.getCount() > 0){
         int pageCount = ((jobNewsPaginationVO.getCount()-1) / jobNewsPaginationVO.getPageSize())+1;
         int startPage = 1;
         int i;
         if (jobNewsPaginationVO.getCurrentPage()%10 !=0)
            startPage = (int)(jobNewsPaginationVO.getCurrentPage()/10)*10+1;
         else
            startPage = jobNewsPaginationVO.getCurrentPage() - 9;
         int pageBlock = 10;

         if(startPage > 10){
%>
            <li class="page-item disabled">
      			<a class="page-link" href="template.templ?page=job_news_list.news?pageNum=<%=startPage - pageBlock %>" tabindex="-1">PREV</a>
    		</li>
...
%>
            <li class="page-item"><a class="page-link" href="template.templ?page=job_news_list.news?pageNum=<%=i %>"><%=i %></a></li>
<%
         }
...
%>
            <li class="page-item">
      			<a class="page-link" href="template.templ?page=job_news_list.news?pageNum=<%=startPage+pageBlock %>">NEXT</a>
    		</li>
