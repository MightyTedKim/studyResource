...
<%@ page import="com.spring.drone.news.LeisureNewsPaginationVO"%> 
...
<% 
   ...
   ArrayList<NewsVO> leisureNewsList = (ArrayList<NewsVO>)request.getAttribute("leisureNewsList");
   LeisureNewsPaginationVO leisureNewsPaginationVO = (LeisureNewsPaginationVO)request.getAttribute("leisureNewsPaginationVO");
   SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
   int number = leisureNewsPaginationVO.getNumber();
...
<%
      if(leisureNewsPaginationVO.getCount() == 0){
%>    
...
<%
      ...
      {
%>
      <table class="table table-sm text_align_center korean_board" style="font-size: 15px; font-weight: bold;">
         <thead>
...
<%
         for(int i=0;i < leisureNewsList.size();i++){
            NewsVO vo = leisureNewsList.get(i);   
%>      
            <tr class="d-flex">
               <td class="col-sm-1">
                  <%=number%>
               ...
               <td class="col-sm-8 pl-0 td_no_overflow">
		          <a href="template.templ?page=getNews.news?num=<%=vo.getNum() %>&pageNum=<%=leisureNewsPaginationVO.getCurrentPage() %>&number=<%=leisureNewsPaginationVO.getNumber() %>&readcount=<%=vo.getReadcount()%>">
                     <%=vo.getTitle() %>
                  </a>
                 </td>
...
<%
      ...
      if(leisureNewsPaginationVO.getCount() > 0){
         int pageCount = ((leisureNewsPaginationVO.getCount()-1) / leisureNewsPaginationVO.getPageSize())+1;
         int startPage = 1;
         int i;
         if (leisureNewsPaginationVO.getCurrentPage()%10 !=0)
            startPage = (int)(leisureNewsPaginationVO.getCurrentPage()/10)*10+1;
         else
            startPage = leisureNewsPaginationVO.getCurrentPage() - 9;
         int pageBlock = 10;

         if(startPage > 10){
%>
            <li class="page-item disabled">
      			<a class="page-link" href="template.templ?page=leisure_news_list.news?pageNum=<%=startPage - pageBlock %>" tabindex="-1">PREV</a>
    		</li>
...
%>
            <li class="page-item"><a class="page-link" href="template.templ?page=leisure_news_list.news?pageNum=<%=i %>"><%=i %></a></li>
<%
         }
...
%>
            <li class="page-item">
      			<a class="page-link" href="template.templ?page=leisure_news_list.news?pageNum=<%=startPage+pageBlock %>">NEXT</a>
    		</li>
